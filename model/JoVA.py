...
class JoVA():
    def __init__(self, sess, args, data):
        ...
        self.x = np.array(range(self.batch_size))
        self.x = tf.convert_to_tensor(np.expand_dims(self.x, 1))
    ...
    def prepare_model(self):
        ...
        self.input_R_N_I = tf.placeholder(dtype=tf.float32, shape=[self.num_users, None], name="input_R_N_I")
        ...
        self.col_N_idx = tf.placeholder(dtype=tf.int32, shape=[None, 1], name="col_idx")
        ...
        vae_loss_u = K.mean(reconstruction_loss_u)
        #######################################################
        inp_tmp = tf.transpose(self.input_R_I)
        inp_N_tmp = tf.transpose(self.input_R_N_I)
        x1Item = Dense(intermediate_dim, activation='tanh')(Input(tensor=(self.num_users,)))
        ...
        z_meanItem_N = encoderItem(inp_N_tmp)[0]
        z_log_varItem_N = encoderItem(inp_N_tmp)[1]
        outputs_N_item = decoderItem(encoderItem(inp_N_tmp)[2])
        dis_loss = K.square(z_mean - z_meanItem) + K.square(K.exp(0.5 * z_log_var) - K.exp(0.5 * z_log_varItem))
        dis_loss = K.sum(dis_loss, axis=-1)
        dis_loss_N = K.square(z_mean - z_meanItem_N) + K.square(K.exp(0.5 * z_log_var) - K.exp(0.5 * z_log_varItem_N))
        dis_loss_N = K.sum(dis_loss_N, axis=-1)
        ranking_loss = K.mean(K.square(dis_loss) + K.exp(-1*dis_loss_N))
        self.I_Decoder = outputs_item
        self.I_N_Decoder = outputs_N_item
        self.Decoder_P =((tf.transpose(tf.gather_nd(tf.transpose(self.U_Decoder), tf.concat(self.col_idx, self.x))))
                        + tf.gather_nd(tf.transpose(self.I_Decoder), tf.concat(self.row_idx, self.x))) / 2.0
        self.Decoder_N = ((tf.transpose(tf.gather_nd(tf.transpose(self.U_Decoder), tf.concat(self.col_N_idx, self.x))))
                          + tf.gather_nd(tf.transpose(self.I_N_Decoder), tf.concat(self.row_idx, self.x))) / 2.0
        bpr_loss = tf.reduce_mean(tf.nn.softplus(-(self.Decoder_P - self.Decoder_N)))
        ...
        reconstruction_loss_N = binary_crossentropy(Input(tensor=inp_N_tmp),
                                                  outputs_N_item)*self.num_users
        #kl_loss = 1 + z_log_varItem - K.square(z_meanItem) - K.exp(z_log_varItem)
        #kl_loss = K.sum(kl_loss, axis=-1)
        #kl_loss *= -0.01
        #vae_loss_i = K.mean(reconstruction_loss + kl_loss)
        vae_loss_i = K.mean(reconstruction_loss)
        vae_loss_i_N = K.mean(reconstruction_loss_N)
        if self.using_bpr == 0:
                self.cost = vae_loss_i + vae_loss_u + vae_loss_i_N + 0.01*ranking_loss
        else:
            self.cost = vae_loss_i + vae_loss_u + vae_loss_i_N + 0.01*ranking_loss + self.beta_value * bpr_loss
    ...
    def train_model(self, itr):
        ...
        batch_cost = 0
        ts = 0
        for i in range(self.num_batch):  # iterate each batch
            row_idx, col_idx, col_N_idx = data_generator.sample()
            input_R_U = self.train_R[row_idx, :]
            input_R_I = self.train_R[:, col_idx]
            input_R_N_I = self.trian_R[:, col_N_idx]

            _, Cost = self.sess.run(
                   [self.optimizer, self.cost],
                   feed_dict={self.input_R_U: input_R_U, self.input_R_I: input_R_I, self.input_R_N_I: input_R_N_I, self.row_idx: np.reshape(row_idx, (len(row_idx), 1)),
                           self.col_idx: np.reshape(col_idx, (len(col_idx), 1)), self.col_N_idx: np.reshape(col_idx, (len(col_N_idx), 1))})
            batch_cost += Cost
        batch_cost /= self.num_batch
    ...
    def test_model(self, itr):
        ...
        Cost, Decoder = self.sess.run(
            ...
            feed_dict={self.input_R_U: self.train_R, self.input_R_I: self.train_R,  self.row_idx: np.reshape(range(self.num_users), (self.num_users, 1)),
